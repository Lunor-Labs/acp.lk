...
export default function StudentDashboard() {
  ...
  async function fetchDashboardData() {
    try {
      ...
      const monthPerf: Record<string, { total: number; count: number }> = {};
      examResults?.forEach((r: any) => {
        const key = new Date(r.submitted_at!).toISOString().slice(0, 7);
        if (!monthPerf[key]) monthPerf[key] = { total: 0, count: 0 };
        monthPerf[key].total += Number(r.percentage);
        monthPerf[key].count += 1;
      });
      ...
      for (let i = 5; i >= 0; i--) {
        ...
        const md = monthPerf[key];
        months.push({ month: label, percentage: md ? Math.round(md.total / md.count) : 0 });
      }
