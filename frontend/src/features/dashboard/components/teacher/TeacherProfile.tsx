...
import { Camera, User, Phone, Mail, Lock, Save } from 'lucide-react';
...
export default function TeacherProfile() {
  ...
  const initials = getInitials(form.full_name || 'T');
      ...
      <div className="max-w-2xl mx-auto space-y-6">
        ...
        <div className="relative rounded-2xl overflow-hidden p-8 flex items-center gap-6" style={{ background: 'linear-gradient(135deg, #1e1e2e 0%, #2d1f3e 100%)' }}>
          ...
          <div className="relative flex-shrink-0">
            {avatarUrl ? (
              <img src={avatarUrl} alt={form.full_name} className="w-24 h-24 rounded-full object-cover border-2 border-white/20" />
            ) : (
          ...
          <div>
            ...
            <span className="inline-flex items-center mt-1.5 px-3 py-1 rounded-full text-xs font-medium text-white/85" style={{ background: 'rgba(255,255,255,0.1)', border: '1px solid rgba(255,255,255,0.15)' }}>
              Teacher
            </span>
          </div>
        </div>
        ...
        <form onSubmit={handleSaveProfile} className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6 space-y-5">
          ...
          <div className="border-t border-gray-100 pt-5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <FormField icon={<User className="w-3.5 h-3.5" />} label="Full Name *" value={form.full_name} onChange={v => setForm(f => ({ ...f, full_name: v }))} placeholder="Your full name" />
              <FormField icon={<Phone className="w-3.5 h-3.5" />} label="Phone" type="tel" value={form.phone} onChange={v => setForm(f => ({ ...f, phone: v }))} placeholder="+94 77 000 0000" />
            </div>
          </div>
